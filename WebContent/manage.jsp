    ...
    for(User user : users)
    {
        %>
            <tr>
                ...
                <td>
                    <a class="btn btn-danger" href="manage.jsp?action=delete&id=<%=user.getuId() %>">Delete</a>
                    <a class="btn btn-warning" href="manage.jsp?action=select&id=<%=user.getuId() %>&username=<%=user.getUsername()%>&password=<%=user.getPassword()%>&sex=<%=user.getSex()%>&des=<%=user.getDescription()%>&type=<%=user.getType()%>">Select</a>
                </td>
            </tr>
        <%
    }
